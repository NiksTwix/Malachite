...
namespace Malachite 
{
	...
	std::vector<PseudoCommand> PseudoByteDecoder::HandleBasicSyntax(const ASTNode& node)
	{
		std::vector<PseudoCommand> result;
	...
	std::vector<PseudoCommand> PseudoByteDecoder::ParseConditionBlock(const ASTNode& node)
	{
		...
		{
			if ((child_node.tokens[0].value.strVal == "if" || child_node.tokens[0].value.strVal == "elif") && child_node.tokens.size() < 4) {		//if/elif (true) - minimum
				Logger::Get().PrintSyntaxError("Condition's checking structure (if,elif) is invalid.", child_node.line);
				continue;
			}
			if (child_node.tokens[0].value.strVal == "else" && child_node.tokens.size() > 1 && child_node.tokens[1].value.strVal != ":") {		//else : or {}, without condition
				Logger::Get().PrintSyntaxError("Condition's checking structure (else) is invalid. Else doesnt have a condition in any form.", child_node.line);
				continue;
			}
			...
			for (int i = 0; i < child_node.tokens.size(); i++)
			{
				...
				if (t.value.strVal == ":")
				{
					if (condition_end_index == SIZE_MAX && child_node.tokens[0].value.strVal != "else") {
						Logger::Get().PrintSyntaxError("Unexpected ':' without condition.", child_node.line);
					}
					else if (child_node.tokens[0].value.strVal == "else") 
					{
			...
			if (child_node.tokens[0].value.strVal != "else")
			{
				...
				auto condition_commands = ex_decoder.DecodeExpression(condition_tokens, compilation_state);
			...
			if (!is_single_string) 
			{
				...
				{
					auto expression_commands = RecursiveHandle(expression);
			...
			else	// if/elif (true): operation
			{
				...
				auto operation_commands = ex_decoder.DecodeExpression(operation_tokens, compilation_state);
			...
			if (child_node.tokens[0].value.strVal != "else")block_commands.push_back(PseudoCommand(PseudoOpCode::Label, { {PseudoCodeInfo::Get().labelID_name, TokenValue(exit_label_id)},{PseudoCodeInfo::Get().labelMark_name, TokenValue("NextConditionLabel")} }));
