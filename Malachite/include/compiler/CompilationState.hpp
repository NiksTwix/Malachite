...
namespace Malachite 
{
	...
	public:
		...
		Function() : return_type(0), function_id(0), name("function") { args = {}; }
	...
	struct CompilationState 
	{
		...
		CompilationState() 
		{
			std::shared_ptr<VisibleFrame> vs = std::make_shared<VisibleFrame>();
			spaces.push_back(vs);
			Type void_type(Type::Category::PRIMITIVE, SyntaxInfoKeywords::Get().typemarker_void);
			void_type.size = 1;
			void_type.vm_analog = Type::VMAnalog::NONE;
			AddTypeToSpace(1, void_type);
			Type int_type(Type::Category::PRIMITIVE, SyntaxInfoKeywords::Get().typemarker_int);
			int_type.size = 8;
			int_type.vm_analog = Type::VMAnalog::INT;
			AddTypeToSpace(1, int_type);
			Type uint_type(Type::Category::PRIMITIVE, SyntaxInfoKeywords::Get().typemarker_uint);
			uint_type.size = 8;
			uint_type.vm_analog = Type::VMAnalog::UINT;
			AddTypeToSpace(1, uint_type);
			Type float_type(Type::Category::PRIMITIVE, SyntaxInfoKeywords::Get().typemarker_float);
			float_type.size = 8;
			float_type.vm_analog = Type::VMAnalog::DOUBLE;
			AddTypeToSpace(1, float_type);
			Type bool_type(Type::Category::PRIMITIVE, SyntaxInfoKeywords::Get().typemarker_bool);
			bool_type.size = 1;
			bool_type.vm_analog = Type::VMAnalog::INT;
			AddTypeToSpace(1, bool_type);
			Type char_type(Type::Category::PRIMITIVE, SyntaxInfoKeywords::Get().typemarker_char);
			char_type.size = 1;
		...
		void AddFunctionToSpace(size_t space, Function& function)
		{
			VisibleFrame* vf = GetSpace(space);
			if (!vf) return;
		...
		VisibleFrame* GetCurrentSpace() { return spaces.size() > 0 ? spaces[spaces.size()-1].get() : nullptr; }
		void PushSpace() 
		{ 
